...
public class WeiXinLoginer {
	...
	public WeiXinLoginer() throws Exception {
			config=new WeiXinConfig();
	}
	
	public String getCodeUrl() {
		return String.format(config.getGetCodeUrl(), config.getAppID(),config.getLoginRedirectUrl(),"snsapi_userinfo",Math.random());
	}
	...
	public WeiXinToken getToken(String code) {
		try {
			WeiXinToken token=WeiXinBase.getWeiXinToken(code);
			if(token==null) {
				String url = String.format(config.getGetTokenUrl(),
						config.getAppID(), config.getAppKEY(), code);
				...
				WeiXinBase.addWeiXinToken(code, token);
			}
			return token;
		}
		catch (Exception e) {
